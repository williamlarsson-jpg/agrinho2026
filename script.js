...
// Dados estruturados do Jogo Simulador
let gameMoney = 100;
let gameEco = 100;
...
const gameStages = [
    {
        ...
        options: [
            { text: "Comprar pesticidas químicos tradicionais (Barato, mas polui a água)", money: -15, eco: -25 },
            { text: "Investir em bio-defensivos e drones de precisão (Mais caro, mas ecológico)", money: -40, eco: +5 }
        ]
    },
    {
        text: "Estágio 2: Você precisa expandir a área de pastagem para o gado da sua agroindústria. O que fazer?",
        options: [
            { text: "Derrubar uma parte da mata nativa da fazenda (Custo zero, mas gera desmatamento)", money: 0, eco: -40 },
            { text: "Implementar o sistema de Integração Lavoura-Pecuária-Floresta (Ganha bônus de sustentabilidade)", money: -30, eco: +20 }
        ]
    },
    {
        text: "Estágio 3: O maquinário da sua indústria está antigo e gastando muito diesel, liberando muito CO₂.",
        options: [
            { text: "Ignorar e manter funcionando assim mesmo (Economiza dinheiro agora)", money: +20, eco: -20 },
            { text: "Financiar novos tratores elétricos e biocombustíveis (Gasto alto, reduz emissões)", money: -50, eco: +20 }
        ]
    }
];
...
// 4. LÓGICA DO MINIGAME SIMULADOR
function initGameLogic() {
    ...
    function updateDashboard() {
        ...
        // Regras de derrota imediatas
        if (gameMoney <= 0) {
            endGame(false, "Sua empresa faliu! O agronegócio exige planejamento financeiro sólido.");
        } else if (gameEco <= 0) {
            endGame(false, "Desastre Ambiental! Sua fazenda causou tanta destruição que foi multada e interditada.");
        }
    }

    function loadStage() {
        if (currentGameStage >= gameStages.length) {
            endGame(true, "Parabéns! Você concluiu a rodada mantendo o negócio lucrativo e respeitando o meio ambiente!");
            return;
        }
        ...
        if(optionsContainer) {
            ...
            stage.options.forEach(option => {
                ...
                btn.onclick = () => {
                    ...
                    gameEco = Math.min(100, gameEco + option.eco); // Máximo 100%
                    currentGameStage++;
                    updateDashboard();
    ...
    function endGame(victory, message) {
        ...
        if(gameOverTitle) gameOverTitle.textContent = victory ? "🏆 Vitória Consciente!" : "🚨 Fim de Jogo!";
    ...
    // Inicializa o game dashboard na primeira carga
    updateDashboard();
    loadStage();
}
